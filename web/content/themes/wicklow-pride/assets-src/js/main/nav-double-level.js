...
const navDoubleLevel = function (menu, options) {
	...
	let defaults = {
		...
		mobileIcon:
			'<svg xmlns="http://www.w3.org/2000/svg" height="24" width="24" class="icon icon--24" focusable="false" aria-hidden="true" fill="currentColor">' +
	...
	function closeSubmenus() {
		let subNavTriggers = Array.prototype.slice.call(
			menu.querySelectorAll('[data-trigger="sub-nav"]')
		);
	...
	function clickHandler(event) {
		if (event.target.matches('[data-trigger="mobile-nav"]')) {
			...
			event.target
				.closest("li")
				.querySelector('[data-trigger="sub-nav"]')
	...
	function closeOnEscKey(event) {
		...
		if (key === "Escape" || key === "Esc" || key === 27) {
			...
			for (let i = 0; i < subNavTriggers.length; i++) {
				if (
					subNavTriggers[i].getAttribute("aria-expanded") === "true"
				) {
					result = false;
					break;
				}
	...
	function menuSetup() {
		container.setAttribute(
			"id",
			"js-click-nav-" + settings.submenuDirection
		);
		if (settings.submenuIntro === true) {
			container.classList.add("js-nav-with-intro");
		}
		const subMenuWrappers = Array.prototype.slice.call(
			menu.querySelectorAll('[data-nav="submenu"], ul.sub-menu')
		);
	...
	function convertLinkToButton(menuItem) {
		...
		if (settings.submenuDirection === "horizontal") {
			...
			if (settings.submenuIntro === true) {
				subMenu.parentNode.insertBefore(
					backButton,
					subMenu.parentNode.children[0]
				);
